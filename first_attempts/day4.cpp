...
uint64_t X_count(uint64_t siz_, std::vector<std::vector<char>> input_) {
    uint64_t count0 {0};
    uint64_t count1 {0};
    uint64_t count2 {0};
    uint64_t count3 {0};
    for (int i {1}; i < siz_-1; ++i) {
        for (int j {1}; j< siz_-1; ++j)
            if (input_.at(i).at(j)=='A') {
                if (input_.at(i-1).at(j-1)=='M' && input_.at(i-1).at(j+1)=='M' && input_.at(i+1).at(j+1)=='S' && input_.at(i+1).at(j-1)=='S') {
                    ++count0;
                    // printf("count0: %llu ", count0);
                }

                if (input_.at(i-1).at(j-1)=='S' && input_.at(i-1).at(j+1)=='M' && input_.at(i+1).at(j-1)=='S' && input_.at(i+1).at(j+1)=='M') {
                    ++count1;
                    // printf("count1: %llu ", count1);
                }

                if (input_.at(i-1).at(j-1)=='S' && input_.at(i-1).at(j+1)=='S' && input_.at(i+1).at(j-1)=='M' && input_.at(i+1).at(j+1)=='M') {
                    ++count2;
                    // printf("count2: %llu ", count2);
                }

                if (input_.at(i-1).at(j-1)=='M' && input_.at(i-1).at(j+1)=='S' && input_.at(i+1).at(j-1)=='M' && input_.at(i+1).at(j+1)=='S') {
                    ++count3;
                    
                }
            }
    }
    // printf("count0: %llu count1: %llu count2: %llu count3: %llu\n", count0, count1, count2, count3);
...
int main() {
    ...
    printf("total xmas: %llu \n", total_count);

    printf("total X-mas: %llu \n", X_count(siz, line_total));
